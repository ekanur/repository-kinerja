...
class db_mysql
{
    ...
    ***************************************************************************/
    ...
    {
    	global $app;
        $app[db_mysql][connection] = @mysql_connect($app[db_mysql][server], $app[db_mysql][username], $app[db_mysql][password]);
		if (!$app[db_mysql][connection]):
			die("koneksi gagal!");
		endif;
		if (!@mysql_select_db($app[db_mysql][name])):
			return FALSE;
		endif;
		return TRUE;
    }
    ...
    ***************************************************************************/
    ...
    {
        global $app;
		$result = mysql_query($sql, $app[db_mysql][connection]);
    	if (eregi("select", $sql)):
    ...
    ***************************************************************************/
    function qry($sql,$error="")
    {
        global $app;		
        $rs = mysql_query($sql, $app[db_mysql][connection]);		
		if (mysql_error()):
			if ($app[debug]):
				$err[] = "SQL : $sql";
				$err[] = "ERROR : " . mysql_error();
    ...
    ***************************************************************************/
    function get_record()
    {
		...
		switch (func_num_args()):
			case 2:
				$sql = "select * from {$app[table_mysql][$args[0]]} where $args[1] limit 1";
				break;
			case 3:
				$sql = "select * from {$app[table_mysql][$args[0]]} where $args[1] = '$args[2]' limit 1";
				break;
		endswitch;
    	db_mysql::query($sql, $rs, $nr);
    	if ($nr):
    	    $record = db_mysql::fetch($rs);
    	else:
    	    $record[0] = "";
    	endif;
		//unset($rs);
    ...
    ***************************************************************************/
    function get_recordset()
    {
		...
		switch (func_num_args()):
			case 1:
				$sql = "select * from {$app[table_mysql][$args[0]]}";
				break;
			case 2:
				$sql = "select * from {$app[table_mysql][$args[0]]} where $args[1]";
				break;
			case 3:
				$sql = "select * from {$app[table_mysql][$args[0]]} where $args[1] = '$args[2]'";
				break;
		endswitch;
    	db_mysql::query($sql, $rs, $nr);
    	return $rs;
    ...
    ***************************************************************************/
    function lookup()
    {
		...
		switch (func_num_args()):
			case 3:
				$sql = "select $args[0] from {$app[table_mysql][$args[1]]} where $args[2] limit 1";
				break;
			case 4:
				$sql = "select $args[0] from {$app[table_mysql][$args[1]]} where $args[2] = '$args[3]' limit 1";
				break;
		endswitch;
    	db_mysql::query($sql, $rs, $nr);
    	if ($nr):
