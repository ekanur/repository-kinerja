...
class DashboardController extends Controller
{
    ...
    public function get_total_tridarma(){
        ...
        if (Session::get('userID_login')==null) {
           $result=array(
              "penelitian_dilitabmas"=>$penelitian_dilitabmas->where("deleted_at", "=", null)->get()->count(),
              "penelitian_non_dilitabmas"=>$penelitian_non_dilitabmas->where("deleted_at", "=", null)->get()->count(),
              "pengabdian_dilitabmas"=>$pengabdian_dilitabmas->where("deleted_at", "=", null)->get()->count(),
              "pengabdian_non_dilitabmas"=>$pengabdian_non_dilitabmas->where("deleted_at", "=", null)->get()->count(),
          );
       }else{
...
public function data_chart_penelitian_dilitabmas(){
  ...
  for ($i=date("Y")-5; $i <= date("Y"); $i++) { 
    ...
    if (Session::get('userID_login')==null) {
     $data[$i]=DB::select("select count(id) from penelitian_dilitabmas where CAST(tahun AS TEXT) like '".$i."' and deleted_at IS NULL ");
      }
      else{
...
public function data_chart_penelitian_non_dilitabmas(){
  ...
  for ($i=date("Y")-5; $i <= date("Y"); $i++) { 
        ...
        if (Session::get('userID_login')==null) {
     $data[$i]=DB::select("select count(id) from penelitian_non_dilitabmas where CAST(tahun AS TEXT) like '".$i."' and deleted_at IS NULL ");
   }else{
...
public function data_chart_pengabdian_dilitabmas(){
  ...
  for ($i=date("Y")-5; $i <= date("Y"); $i++) { 
      ...
      if (Session::get('userID_login')==null) {
     $data[$i]=DB::select("select count(id) from pengabdian_dilitabmas where CAST(tahun AS TEXT) like '".$i."' and deleted_at IS NULL ");
   }else{
...
public function data_chart_pengabdian_non_dilitabmas(){
   ...
   for ($i=date("Y")-5; $i <= date("Y"); $i++) { 
     ...
     if (Session::get('userID_login')==null) {
     $data[$i]=DB::select("select count(id) from pengabdian_non_dilitabmas where CAST(tahun AS TEXT) like '".$i."' and deleted_at IS NULL ");
   }else{
